...
class Querys 
{
  ...
  public function GetTransactions ($authToken, $transaccionId = "", $fechaDesde = "", $fechaHasta = "", $estadoTransaccion = "", $transaccionComercioId = "", $numeroSucursal = "", $caja = "", $pagina = "", $cantidad = "")
  {
		...
		$querys = sprintf("?transaccionId=%s&fechaDesde=%s&fechaHasta=%s&estadoTransaccion=%s&transaccionComercioId=%s&numeroSucursal=%s&caja=%s&pagina=%s&cantidad=%s", $transaccionId, $fechaDesde, $fechaHasta, $estadoTransaccion, rawurlencode($transaccionComercioId), $numeroSucursal, $caja, $pagina, $cantidad);
  ...
  public function GetTransactionByTxComercioId($authToken, $transaccionComercioId = "")
  {
		...
		$this->restClient = new \PPConnectorSDK\Services\RESTClient($this->endpoint, $new_headers);
		$RESTResponse = $this->restClient->get(sprintf("transaction/%s", rawurlencode($transaccionComercioId)), "");
		return $RESTResponse;
  }
  ...
  public function GetTransactionsAgrupador($authToken, $ente = "", $pagina = "", $cantidad = "")
  {
		...
		$this->restClient = new \PPConnectorSDK\Services\RESTClient($this->endpoint, $new_headers);
		$RESTResponse = $this->restClient->get("grouper-transactions?ente=".rawurlencode($ente)."&pagina=".$pagina."&cantidad=".$cantidad, "");
		return $RESTResponse;
  }
  ...
  public function GetTransactionByTxComercioIdAgrupador ($authToken, $transaccionComercioId)
  {
		...
		$this->restClient = new \PPConnectorSDK\Services\RESTClient($this->endpoint, $new_headers);
		$RESTResponse = $this->restClient->get("grouper-transaction/".rawurlencode($transaccionComercioId), "");
		return $RESTResponse;
  }
  ...
  public function GetDataComercio($authToken, $ente, $cuit, $qrData)
  {
		...
		$this->restClient = new \PPConnectorSDK\Services\RESTClient($this->endpoint, $new_headers);
		$RESTResponse = $this->restClient->get("get-data-comercio?ente=".rawurlencode($ente)."&cuit=".$cuit."&qrData=".$qrData, "");
		return $RESTResponse;
  }
